...
import { generateAsciiArt } from '~/lib/ascii-fonts'
...
const COMMAND_HELP: Record<string, { usage: string; description: string; examples?: string[] }> = {
  help: { usage: 'help [command]', description: 'Display help information. Use "help <command>" for detailed info on a specific command.', examples: ['help', 'help cd', 'help grep'] },
  cd: { usage: 'cd <directory>', description: 'Navigate to another page. Use "cd .." to go up, "cd ~" or "cd home" to go to homepage.', examples: ['cd blog', 'cd portfolio', 'cd ..', 'cd ~'] },
  ls: { usage: 'ls', description: 'List all available top-level pages on the site.' },
  pwd: { usage: 'pwd', description: 'Print the current working directory (your current page path).' },
  cat: { usage: 'cat <filename>', description: 'Display the first few lines of a blog post or portfolio item content.', examples: ['cat building_tileforge', 'cat safe-route'] },
  grep: { usage: 'grep <term>', description: 'Search posts by title and tags.', examples: ['grep rust', 'grep typescript'] },
  man: { usage: 'man <slug>', description: 'Display metadata for a post including title, date, tags, and location.', examples: ['man building_tileforge'] },
  tree: { usage: 'tree', description: 'Display the full site structure as a tree diagram.' },
  history: { usage: 'history', description: 'Show your command history for this session.' },
  ascii: { usage: 'ascii <text>', description: 'Generate ASCII art from text (max 12 characters).', examples: ['ascii hello', 'ascii rust'] },
  neofetch: { usage: 'neofetch', description: 'Display system information in the classic neofetch style.' },
  matrix: { usage: 'matrix', description: 'Enter the matrix. Press any key or click to exit.' },
  fireworks: { usage: 'fireworks', description: 'Launch a fireworks display. Press any key or click to exit.' },
  confetti: { usage: 'confetti', description: 'Celebrate with confetti!' },
  mode: { usage: 'mode [light|dark]', description: 'Toggle or set the color mode (light/dark).', examples: ['mode', 'mode light', 'mode dark'] },
  rain: { usage: 'rain', description: 'Watch the digital rain fall. Press any key or click to exit.' },
  clear: { usage: 'clear', description: 'Clear the terminal screen.' },
  github: { usage: 'github', description: 'Open the GitHub profile in a new tab.' },
  x: { usage: 'x', description: 'Open the X (Twitter) profile in a new tab.' },
  whoami: { usage: 'whoami', description: 'Display information about the site author.' },
  echo: { usage: 'echo <message>', description: 'Echo back the provided message.', examples: ['echo hello world'] },
  contact: { usage: 'contact', description: 'Display contact information including email, LinkedIn, GitHub, and X.' },
  theme: { usage: 'theme [name]', description: 'Switch the site theme. Run without arguments to see available themes.', examples: ['theme', 'theme dracula', 'theme nord'] },
  particles: { usage: 'particles [off|low|medium|high]', description: 'Set the cursor particle effect density. Run without arguments to see current setting.', examples: ['particles', 'particles high', 'particles off'] },
}
...
└── uses`
    ...
    case 'neofetch': {
      ...
      const info = [
        ...
        '─────────────────',
        `<span style="color:#d79921">OS</span>: TanStack Start`,
...
export const Route = createFileRoute('/api/commands')({
  server: {
    ...
    const firstArgs = pipeSegments[0].split(/\s+/).filter(Boolean)
